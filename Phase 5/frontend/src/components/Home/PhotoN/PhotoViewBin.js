...
import PhotoActionsBinContainer from '../../PhotoActionsBin';

const PhotoDetailsBin = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        axios
            .get(`http://localhost:5001/api/bin/photo-details/${id}`)
            .then((res) => {
    ...
    if (!photo) {
        ...
        return (
            <div>
                ...
                <PhotoActionsBinContainer photo={photo}  />
            </div>
        );
...
export default PhotoDetailsBin;
