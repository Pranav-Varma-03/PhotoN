...
const AlbumUploadModel = require('../models/AlbumUploadModel');
const router = Router();

router.get("/api/get/view-album-shared", async (req, res) => {
    ...
    try {
        ...
        const sharedPhotoRows = await sharedAlbum.find({ sharedUserId: { $in: user._id } });
        const photoIds = sharedPhotoRows.map((row) => row.albumId);
        const photosData = await AlbumUploadModel.find({ _id: { $in: photoIds } });
